    ...
    <Tabs value={currentTab} onValueChange={(v) => setTab(v as TabKey)}>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium mt-0.5 text-sm leading-relaxed">
                    {member.address ?? "-"}
                  </p>
                </div>
        ...
        <div className="flex justify-end">
          <Dialog open={showAssign} onOpenChange={setShowAssign}>
            <DialogTrigger asChild>
              <Button size="sm">
                <Plus className="mr-2 h-4 w-4" /> Assign Paket
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-md">
              ...
              <AssignPackageForm memberId={member.memberId} onSuccess={() => setShowAssign(false)} />
            </DialogContent>
          </Dialog>
        </div>
        {pkgLoading ? (
          ...
          <div className="space-y-3">
            {packages.map((pkg) => (
              <PackageCard key={pkg.memberPackageId} pkg={pkg} memberId={member.memberId} />
            ))}
          </div>
        )}
      </TabsContent>
      ...
      <TabsContent value="documents" className="mt-4 space-y-4">
        <Dialog open={showUpload} onOpenChange={setShowUpload}>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <DocumentUploadForm memberId={member.memberId} onSuccess={() => setShowUpload(false)} />
          </DialogContent>
        </Dialog>
        {docLoading ? (
          ...
          <DocumentList documents={documents ?? []} memberId={member.memberId} />
        )}
      </TabsContent>
      ...
      <TabsContent value="branch-access" className="mt-4 space-y-4">
        ...
        {baLoading ? (
          ...
          <BranchAccessList accesses={accesses ?? []} memberId={member.memberId} />
        )}
      </TabsContent>
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">Dr. {entry.doctor.fullName}</p>
                    </div>
                    {entry.diagnosis && (
                      <Badge variant="secondary" className="text-xs">{entry.diagnosis}</Badge>
                    )}
                  </div>
                </CardHeader>
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        {entry.sessions.map((s) => (
                          <li key={s.sessionId} className="text-xs text-slate-600 dark:text-slate-400 flex gap-2">
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {entry.emrNotes.map((note) => (
                          <li key={note.noteId} className="rounded-md bg-slate-50 dark:bg-slate-800 p-3">
