...
import { NotificationSidebarBadge }  from '@/components/modules/notification/NotificationSidebarBadge'
...
// ─── Shared badge instances ───────────────────────────────────────────────────

const STOCK_BADGE = <StockAlertBadge />
...
const NAV_ITEMS: Record<RoleName, NavItem[]> = {
  ...
  MASTER_ADMIN: [
    ...
    { label: 'Audit Log', href: 'masteradmin/audit-logs', icon: ClipboardList   },
  ],
  ADMIN: [
    ...
    { label: 'Inventori',       href: 'admin/inventory',      icon: Boxes,       badge: STOCK_BADGE      },
    { label: 'Permintaan Stok', href: 'admin/stock-requests', icon: ShoppingCart                         },
  ...
  NURSE: [
    ...
    { label: 'Stok Kritis',     href: 'nurse/inventory',       icon: FlaskConical, badge: STOCK_BADGE },
...
const SHARED_ITEMS: NavItem[] = [
  {
    ...
    icon:  Bell,
    badge: <NotificationSidebarBadge />,
  },
  {
    label: 'Chat',
    href:  'shared/chat',
...
// ─── Accent warna per role ────────────────────────────────────────────────────

const ROLE_ACCENT: Record<RoleName, string> = {
  SUPER_ADMIN:  'text-violet-600 bg-violet-50  dark:bg-violet-950 dark:text-violet-400',
  MASTER_ADMIN: 'text-blue-600   bg-blue-50    dark:bg-blue-950   dark:text-blue-400',
  ADMIN:       'text-emerald-600 bg-emerald-50 dark:bg-emerald-950 dark:text-emerald-400',
    ...
    >
      ...
      <div className="flex h-16 items-center justify-between px-4 border-b border-slate-200 dark:border-slate-700">
        ...
        <Button
          ...
          className="h-8 w-8 ml-auto text-slate-500 hover:text-slate-900 dark:hover:text-slate-100"
        >
          {sidebarOpen
